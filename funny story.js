...
var pages = {
  ...
  second:{
    ...
    content: (getData, setData) => (
      <p>
        <h1><strong>You think it's over? Come on!</strong></h1>
  ...
  welcome: {
    content: (getData, setData) => (
      <p>
        Welcome, <strong>{getData("name")}</strong>, <strong>{getData("birthday")}</strong>! How would you like to
  ...
  onthetrain: {
    content: (getData, setName) => (
      <p>
        Welcome aboard the <strong>{getData("number")}</strong>  train! Please make your way to your seat.
        We are going to tell a story!
      </p>
    ),
    buttons: [{ label: "Let's do it", page: "story" }, { label: "Cool!", page: "story" }]
  },
  ontheship: {
    content: (getData, setName) => (
      <p>
        Welcome to the <strong>{getData("number")}</strong>  Ship! Please find your seat.
        Are you ready? We will take off to the funny story kingdom very soon!
      </p>
    ),
    buttons: [{ label: "Yes", page: "food" }, { label: "No,story please!", page: "story" }]
  },
 food: {
    content: (getData, setName) => (
      <p>
    ...
    buttons: [{ label: "Thanks", page: "story" }, { label: "i'm fine", page: "story" }]
  },
  story: {
    content: (getData, setName) => (
      <p>
        It was a  <strong>{getData("adjective")} </strong>,cold November day.I woke up to the <strong>{getData("aj2")} </strong> smell of <strong>{getData("bird")} </strong> roasting in the <strong>{getData("room")} </strong> downstairs.I <strong>{getData("verbp")} </strong> down the stairs to see if I could help <strong>{getData("verb")} </strong> the dinner.
        My Mom said,"see if <strong>{getData("rela")} </strong> needs a fresh <strong>{getData("noun")} </strong>." So I carried a tray of glasses full of <strong>{getData("liquid")} </strong> into the <strong>{getData("verbing")} </strong> room.When I got there,I couldn't believe my <strong>{getData("body")} </strong>!
        There were <strong>{getData("pnoun")} </strong> <strong>{getData("verbing")} </strong> on the <strong>{getData("noun")} </strong>.
      </p>
    ),
    buttons: [{ label: "Boring!!!", page: "welcome" }, { label: "hahahaha", page: "welcome" }]
  }
};
